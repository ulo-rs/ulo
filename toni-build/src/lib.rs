...
//! implementing `toni_grpc::MethodShape` — that the macro projects through:
//!
//! ```ignore
//! // build.rs
...
const SENTINEL: &str = "// ==== toni-build: what each method carries, for #[grpc_methods] ====";
...
fn companion_for(service: &ItemTrait) -> Result<TokenStream, Error> {
    ...
    for item in &service.items {
        ...
        let install = if is_streaming(&arg) {
            quote! { ::toni_grpc::shape::stream(request, ctx) }
        } else {
            quote! { ::toni_grpc::shape::message(request, ctx) }
        };
        markers.extend(quote! {
            pub struct #marker;
            impl ::toni_grpc::MethodShape for #marker {
                type Arg = #arg;
                fn install(
    ...
    let module = format_ident!("{}_toni", to_snake(&trait_name));
    let doc = format!(
        " What each method of `{trait_name}` carries, read off the trait tonic \
         generated. Written by toni-build; `#[grpc_methods]` projects through these."
    );
    Ok(quote! {
        #[doc = #doc]
        pub mod #module {
...
/// tonic-build's own snake-casing, so `Greeter` names `greeter_toni` beside
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::NoOutDir => write!(
                f,
                "toni_build::shapes reads $OUT_DIR, which cargo sets only for a build script"
            ),
...
mod tests {
    ...
    fn every_call_shape_gets_a_marker_naming_what_the_wire_carries() {
        ...
        assert!(out.contains("pub mod greeter_toni"), "{out}");
        for (marker, arg, install) in [
            ...
            assert!(
                out.contains(&format!("impl ::toni_grpc::MethodShape for {marker}")),
                "{marker}: {out}"
            );
            assert!(
            ...
            assert!(out.contains(&format!("::toni_grpc::shape::{install}(request, ctx)")));
        }
        assert!(
    ...
    fn a_second_run_replaces_its_own_section() {
        ...
        assert_eq!(twice.matches("pub mod greeter_toni").count(), 1);
    }
