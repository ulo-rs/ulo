    ...
    let expanded = quote! {
        {
            ...
            impl ulo::traits_helpers::ProviderFactory for #wrapper_factory_name {
                ...
                async fn build(
                    ...
                    let ulo::traits_helpers::Injectable { instance: inner_provider, roles } = #type_path::__ulo_provider_factory().build(deps).await;

                    // Wrap under the custom token; forward roles unchanged.
                    #[derive(Clone)]
                    struct CustomTokenProvider {
                        custom_token: String,
                        inner_provider: std::sync::Arc<Box<dyn ulo::traits_helpers::Provider>>,
                    }
                    ...
                    ulo::traits_helpers::Injectable::new(provider, roles)
                }
            }
