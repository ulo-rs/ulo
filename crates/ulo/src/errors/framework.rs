...
/// before the transport's own rejection is rendered — 403 on HTTP; on RPC, the canonical
/// `Forbidden` envelope on the `response` lane; `PermissionDenied` on gRPC; the canonical envelope
/// on WebSocket.
///
