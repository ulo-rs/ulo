...
Redis Pub/Sub adapter for cross-process WebSocket broadcasting in the [Ulo](https://github.com/ulo-rs/ulo) framework.

## The problem

Ulo's built-in `BroadcastService` is in-process. When you run multiple instances of your application behind a load balancer, clients land on different processes. A broadcast call on process A only reaches clients connected to process A — clients on process B never see it.
...
rbs.to_room("lobby")  ──►  PUBLISH                ──►  deliver to
  .send(msg)               ulo:broadcast               local clients
                                                         in "lobby"
```
...
// Wrap in {"event": "...", "data": ...} format expected by ulo's gateway router
