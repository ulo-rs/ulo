//! An RPC server and a client over Kafka.
//!
//! Kafka has no reply channel, so the client names a private reply topic
//! in the `ulo-reply-to` header and correlates by `ulo-correlation-id`. Both
//! are stripped before the handler sees the call's headers.
//!
...
//!     KAFKA_BROKERS=127.0.0.1:9092 cargo run -p ulo-rpc-kafka --example orders
...
async fn main() -> anyhow::Result<()> {
    let endpoint = std::env::var("KAFKA_BROKERS").unwrap_or_else(|_| "127.0.0.1:9092".into());
        ...
        .run_until(async move {
            let server = {
                ...
                tokio::task::spawn_local(async move {
                    ...
                    app.use_rpc_adapter(KafkaAdapter::new(endpoint.clone()))
                        .unwrap();
                    app.bind().await.unwrap();
                    println!("serving order.* over Kafka");
                    app.run().await;
                })
            };
            ...
            let client = RpcClient::new(KafkaClientTransport::new(endpoint.clone()));
