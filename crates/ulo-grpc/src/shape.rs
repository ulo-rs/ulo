...
//! projects through a marker ulo-build wrote beside the trait:
//!
//! ```ignore
//! async fn greet(&self, request: ::tonic::Request<<greeter_ulo::Greet as MethodShape>::Arg>)
...
/// its markers the same way, from `ulo_build::shapes_in_file`.
...
impl<T: Send + 'static> RequestCarrier for StreamRequest<T> {
    ...
    /// sees ulo's, so the conversion happens once here.
    fn take_message(self: Box<Self>) -> Box<dyn Any + Send> {
        let items = ulo::futures::StreamExt::map(self.0.into_inner(), |item| {
            item.map_err(|status| {
                GrpcStatus::new(
