//! An RPC server and a client over Redis.
//!
//! Redis Pub/Sub carries no request-response of its own: the client
//! subscribes to a private reply channel and the server publishes there,
//! matched by a correlation id.
//!
//! The handler side is the same code every transport runs — `#[controller]`
//! plus `#[patterns]`, with the pattern naming the channel. What changes
