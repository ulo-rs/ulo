...
/// Consume a poem `Request`, returning ulo's parts + body pair. Path params
...
fn split_request(req: PoemRequest) -> (RequestPart, RequestBody) {
    ...
    let ulo_body = poem_body_to_ulo(body);
    (http_parts, ulo_body)
}

/// Wrap a poem `Body` (an `http_body_util::BoxBody<Bytes, IoError>`) as ulo's
/// `RequestBoxBody` (`UnsyncBoxBody<Bytes, Box<dyn Error + Send + Sync>>`).
fn poem_body_to_ulo(body: PoemBody) -> RequestBody {
...
fn ulo_response_to_poem(http_res: HttpResponse) -> PoemResponse {
    ...
    // ulo's response body is `Send + !Sync`; poem's `Body::from(BoxBody)`
    ...
    let body = match http_res.body {
        None => PoemBody::empty(),
        Some(ulo_body) if ulo_body.is_streaming() => PoemBody::from_bytes_stream(
            ulo_body
                .into_box_body()
                .into_data_stream()
                .map_err(std::io::Error::other),
        ),
        Some(ulo_body) => PoemBody::from_bytes(ulo_body.try_bytes().cloned().unwrap_or_default()),
    };
...
/// Rebuild a poem `Body` from ulo's, preserving streaming.
fn ulo_body_to_poem(body: RequestBody) -> PoemBody {
    match body {
...
fn poem_response_to_ulo(res: PoemResponse) -> HttpResponse {
...
/// Poem endpoint that bridges a single ulo HTTP route.
struct UloEndpoint {
    handler: Arc<dyn RequestHandler>,
}

impl Endpoint for UloEndpoint {
    ...
    async fn call(&self, req: PoemRequest) -> poem::Result<Self::Output> {
        ...
        Ok(ulo_response_to_poem(http_res))
    }
}

/// Convert ulo `{param}` path segments to poem's `:param` syntax. `:param`
...
impl Endpoint for GlobalChainEndpoint {
    ...
    async fn call(&self, mut req: PoemRequest) -> poem::Result<Self::Output> {
        ...
        let http_req = HttpRequest::from_parts(parts, poem_body_to_ulo(body));
            ...
            .execute(http_req, move |treq| {
                Box::pin(async move {
                    ...
                    *req.extensions_mut() = parts.extensions;
                    req.set_body(ulo_body_to_poem(body));
                    ...
                    match inner.call(req).await {
                        Ok(res) => poem_response_to_ulo(res),
                        // No catch-all route hosts the ulo 404: a `/*` route
                        ...
                        Err(err) => poem_response_to_ulo(err.into_response()),
                    }
                })
            })
            .await;
...
/// connection through ulo's `WsConnectionCallbacks`.
struct UloWsEndpoint {
    callbacks: Arc<WsConnectionCallbacks>,
}

impl Endpoint for UloWsEndpoint {
    type Output = PoemResponse;
...
async fn run_ws_connection(
    ...
    while let Some(result) = read.next().await {
        match result {
            ...
            Ok(poem::web::websocket::Message::Close(_)) => {
...
fn build_method_routes(
    ...
    for (method, path, handler) in routes {
        let endpoint = UloEndpoint { handler }.map_to_response().boxed();
...
impl HttpAdapter for PoemAdapter {
    ...
    async fn into_lifecycle(
        ...
        for (path, callbacks) in ws_routes {
            let ws_endpoint = UloWsEndpoint { callbacks };
